...
2. **Infrastructure execution:** The configuration files are executed, creating QEMU/KVM virtual machines connected through virtual or physical network bridges.
...
2. Install KVM, QEMU and LibVirt on each physical machine (QEMU v4.2.1 or v6.1.0).<br>
    ...
    * The [virtual network bridge](https://wiki.libvirt.org/page/Networking) (see `virsh net-list`) should expose the IP range 192.168.122.*
    * When using LibVirt for infrastructure deployment on multi physical machines, a physical network bridge is required. See `docs/NETWORK.md` for installation steps.
