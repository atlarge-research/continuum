...
# TODO controller should be found like this
# def get_controller(package):
#     return package['controller']


def copy():
    # TODO (high priority)
    #  - see comments in ansible.py on copying
    logging.info("Start copying Kubernetes files to all hosts")

    dest = os.path.join(settings.config["base_path"], ".continuum/")
    out = []

    # Copy inventory file to all hosts - just copy to all hosts, not needed to select
    for host in settings.config["hosts"]:
        source = os.path.join(settings.config["base"], f".tmp/{file}")
        result = settings.copy_files(source, dest, host)
        out.append(result)
