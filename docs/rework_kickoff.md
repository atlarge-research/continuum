...
6. Treat Phase E resume/state integrity as landed for the active rework scope:
...
7. Treat Phase F test architecture closure as landed:
   ...
   - unit tests live under `scripts/test/unit/`,
   - local e2e-runner regression tests live under `scripts/test/e2e/`,
   - shared test helpers live under `scripts/test/support/`.
8. Latest validation baseline is green:
...
9. Use this quick validation set before handing off again:
...
10. Focused validation from the Phase-D prep slice is also green:
