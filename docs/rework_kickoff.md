...
7. PR-4 prep has progressed by replacing core gated runtime uses of removed `workload_*`/legacy benchmark sizing helpers with generic benchmark-pipeline parameter access helpers (`benchmark_param*`), including `application/*` module callsites.
...
13. PR-4 runtime helper plumbing now includes `config_access` readers for benchmark-stage planner assignments, and gated Kubernetes helper paths pass planner-derived stage handoff metadata forward without inferring application role topology from a single selector.
...
22. PR-4 benchmark handoff access now exposes canonical runtime bundles (`benchmark_stage_handoff` / `benchmark_stage_handoffs`) with planner assignment ids, pipeline indexes, deep-copied stage config, resolved resources, scope identities, tags, and tier counts; gated Kubernetes launch variables pass primary and pipeline-ordered handoff data forward without enabling application execution.
...
25. PR-4 gated Kubernetes launch variables now forward a combined `planner_runtime_handoff` payload containing ordered software-module and benchmark-stage config/placement metadata, giving Phase-D role/template work one planner-derived input surface without ungating application execution.
...
27. PR-4 is complete; next work is PR-5 examples/smokes/documentation closure while the Phase-D application execution gate remains closed.
...
33. PR-5 smoke policy is now documented as fast-fail, artifact-retaining, and phase-oriented: Kubernetes is the canonical software smoke target, netperf/network validation stays separate, and the preferred future benchmark smoke path is a resumed multi-phase run on reused VMs once Phase D opens.
