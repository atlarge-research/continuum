...
| --- | --- | --- |
| `configuration/tests/qemu/` | Most old-main QEMU parity rows have explicit YAML equivalents under `configs/experiments/parity/` and release status in `docs/release_certification_matrix.md`. Exact full `P-QEMU-10` remains `ported-unverified`. | Keep exact `P-QEMU-10` unclaimed until exact-resource VM/application evidence exists; otherwise maintain the certified parity YAML and evidence docs. |
...
| `configuration/experiment_control/` | Legacy kubecontrol microbenchmarks are not release-certified. The smallest future target is documented in the release matrix. | Add a local-QEMU YAML equivalent for one minimal kubecontrol plus `empty` benchmark first, then evidence cluster readiness, metrics, app success, and artifacts before any claim. |
| `configuration/experiment_kata/` | Legacy Kata startup and resource-usage sweeps are not release-certified. The smallest future target is documented in the release matrix. | Add a local-QEMU YAML equivalent for one minimal `kube_kata` plus `empty_kata` startup benchmark first, with host prerequisites and Kata runtime evidence. Decide separately whether resource-usage sweeps become YAML examples or historical artifacts. |
