...
`/home/matthijs/continuum/logs/cloud_static_audit/cloud_static_audit_2026-05-25T181452Z.md`
...
10. pre-tag issues: 9 expected blockers.
...
`python3 scripts/test/check_release_pretag.py` currently reports 9 expected
blockers:
...
1. `pretag-host-helper-not-ready`: M1 evidence still records
   ...
   `PASS`.
2. `pretag-source-tree-dirty`: the worktree is dirty.
...
tree, refreshed host-helper verification, fresh evidence from the exact source
tree being tagged, and rerunning the pre-tag checker until it reports zero
issues.
