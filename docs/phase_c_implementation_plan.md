...
6. PR-4 prep progressed: core gated runtime surfaces (`application/application.py`, `resource_manager/kubernetes/kubernetes.py`, `resource_manager/endpoint/endpoint.py`) now consume `domains.benchmark.pipeline` via generic `config_access.benchmark_param*` helpers instead of removed `workload_*` and legacy benchmark sizing helpers.
...
56. Experiment lock writing now persists an additive `planner_snapshot` with deterministic software execution order, owner-tagged plan entries, and benchmark-stage assignment metadata to support PR-4 benchmark handoff preparation while application execution remains runtime-gated.
...
60. Benchmark handoff helper wiring now exposes planner-stage assignments and resolved resources through `config_access`, and gated Kubernetes helper paths pass planner-derived stage handoff metadata forward without inferring application role topology from a single selector.
...
70. Gated Kubernetes launch-variable preparation now forwards the primary benchmark handoff bundle, pipeline-ordered handoff bundles, and flattened compatibility fields, preparing Phase-D role/template consumers without deriving application role topology from a single selector or enabling `run.targets: application`.
...
72. Gated Kubernetes launch-variable preparation now forwards a combined `planner_runtime_handoff` payload alongside compatibility fields so Phase-D role/template consumers can read ordered planner-derived software and benchmark config/placement from one runtime object.
73. Phase-D application role consolidation should split application-specific launch/timing/runtime concerns out of `resource_manager/kubernetes/kubernetes.py` (`launch_with_starttime`, MQTT environment injection, and Mist/Baremetal worker-output handling) once application execution is ungated.
74. PR-4 runtime handoff bundles now preserve their original ordering via explicit `pipeline_index` / `module_index` fields and include deep-copied stage/module config, making the Phase-D template handoff self-contained without opening the application execution gate.
...
3. Next implementation focus is Phase D application-role consolidation plus the resumed K8s smoke pipeline/teardown verification work that depends on Phase-D runtime ungating.
