...
| `openfaas` | YAML profile, suite, and a single-host CPU-capped software-only variant have VM-backed software-phase evidence. | `certified` for `P-QEMU-10-SW-LOCAL` only | Full application evidence after root-helper cache priming and exact-resource-capacity decisions are resolved. |
