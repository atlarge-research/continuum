...
| `kubeedge` | YAML profiles exist; a software-only legacy-topology suite has VM-backed software-phase evidence on the legacy topology; the full application suite is ported and can use the host-primed local-registry cache. | `certified` for `P-QEMU-06-SW` only | Move or prune retained smoke state, then rerun full application evidence with metric artifacts. |
...
7. Move or prune retained smoke state, then rerun the full `P-QEMU-06`
   application evidence path after successful host-side registry cache priming.
