...
| `openfaas` | YAML profile, suite, single-host CPU-capped software/application variants, and the exact OpenFaaS image-classification parent row have VM-backed evidence. | `certified` for `P-QEMU-10`, `P-QEMU-10-SW-LOCAL`, and `P-QEMU-10-APP-LOCAL` only | Fresh VM/cloud evidence per additional OpenFaaS topology or provider claim. |
