...
def get_image_location(self):
    ...
    """
    # Find the execution mode
    benchmark = settings.get_benchmark()
    is_openfaas = False
    for layer in benchmark["layer"]:
        if "openfaas" in layer:
            is_openfaas = True
...
def verify_options(parser, config):
    ...
    Args:
        ...
        config (ConfigParser): ConfigParser object
    """
    # TODO - deployment modes (e.g., serverless does not have combined)
    ...
    for i, out in enumerate(worker_output):
        ...
        for line in out:
            if start_time == 0 and "Read image and apply ML" in line:
                start_time = workload.to_datetime(line)
            elif "Get item" in line:
                end_time = workload.to_datetime(line)
...
def gather_endpoint_metrics(config, endpoint_output, container_names):
    ...
    for out, container_name in zip(endpoint_output, container_names):
        ...
        start_time = workload.to_datetime(out[0])
        end_time = workload.to_datetime(out[-1])
