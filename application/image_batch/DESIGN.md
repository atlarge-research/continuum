...
The demo is a scientific vertical slice of an eventual closed-loop digital twin. It must create real endpoint-to-cloud traffic, execute measurable work on Kubernetes, and preserve enough evidence to reconstruct completed work and the cluster state at a chosen cutoff. That evidence supplies causal arrival forecasts, OpenDC inputs and a repeated capacity controller.

The demo observes and forecasts workload, simulates worker-count alternatives, and repeatedly controls worker admission on the real cluster. Live-state reconstruction remains an approximation with explicit validity checks. Its purpose is to test modeling assumptions under controlled conditions; reproducible timing and trace correctness take priority over production fault tolerance.
...
After both a successful workload profile and its emission record have been flushed, recurring snapshots stop repeating that terminal Job and its Pods. Raw Kubernetes lists are still fetched, but archived identities are filtered before expensive SDK object construction. Finalization of resource samples alone does not retire a Job, and failed, malformed or unemitted work remains observable. Append-only arrival and completion streams preserve the full historical inventory; the final experiment archive also retains the complete Kubernetes Job and Pod lists. This keeps observation cost tied mainly to unfinished work without treating absent terminal profiles as known completions.
...
The same sampled futures are used across worker-count candidates. The selected scale-down worker and its assigned Jobs are omitted and retained for later isolated execution; remaining-worker results are explicitly partial. The preliminary evaluation PDF compares a fixed window with following the same included cohort through completion. Its partial totals cannot select a policy; the closed-loop controller uses the separate pinned path with complete represented-work accounting.
...
The optional pinned trace path requests each represented running remainder or starting profile on its observed worker at time zero. Assigned tasks precede ordinary work in the adapted native trace; original source ordering and timestamps remain preserved. Admission checks aggregate CPU and memory on each named worker before execution, because a placement override must not bypass resource fit. Native lifecycle validation then checks initial scheduling, host identity throughout execution, and capacity. This restores placement for represented work, not the entire observed state. The classifier-only replay retains its historical limitations; the closed-loop occupancy model adds causal startup, release and exhausted-work estimates while keeping unresolved membership explicit.

Native cordon retains all executable tasks and initial workers. The selected worker finishes its assigned work without admitting queued or future tasks, then closes in the simulator. Selection keeps the same observed-empty/earliest-assignment heuristic as the provisional path. Closed-loop down candidates are unavailable whenever exhausted or unresolved work prevents reliable drain accounting. Previously cordoned workers remain unavailable to new work in every candidate, including the unchanged case. A complete modeled task inventory still does not establish complete physical observation or validate the scaling counterfactual. Kubernetes cordon alone does not power off a VM.
...
Compatible action/sample matrices can share one sequential native process. A common topology closes absent reserve hosts immediately; every action retains the same initial task inventory and each sampled future has an explicit identity, even when trace bytes coincide. This optimization requires agreement with individual execution on placement, timing and energy, not merely a successful exit. Shared wall time, CPU and peak memory belong to the whole experiment and cannot be divided into measured per-sample costs. Empty members are completed analytically with explicit provenance; compatible nonempty members share the native process, while incompatible matrices retain individual execution. Observation validation accepts unchanged cases only, preventing action variants from being pooled as if they were alternative forecasts of an unchanged observed run.

## Repeated capacity control

The controller closes the observation–prediction–action cycle while the endpoint workload remains open-loop: request times do not adapt to service response. Each tick freezes an observation prefix, fits arrivals only from evidence already available, evaluates identical sampled futures across capacity candidates, chooses an action, rereads the physical state, and observes the result. Native computation runs on the control plane with its own resource limit. Application capacity remains worker-only, with C−1 applied once.

The initial policy minimizes accepting-plus-draining application core-time subject to a Job response constraint measured from original creation. Every sampled future must predict at least 95% completion within 120 seconds; a down action also needs at least 10% predicted allocation saving and two consecutive wins for the same target. A previous winning target is retained for fresh simulation while it remains eligible, avoiding arbitrary candidate oscillation. This carries a selection preference, not a cached score. Ties retain capacity. If no candidate meets the response constraint, an up action must improve the worst late fraction, then mean tardiness, relative to holding; otherwise the controller records infeasibility.

Capacity choices are hold, cordon one accepting worker without eviction, or uncordon one empty Ready reserve. Existing assigned work drains in place and continues to count as allocation. Only one worker may drain at a time, worker bounds are explicit, and a cooldown limits repeated changes. Powered warm reserves are an availability assumption, not an energy-saving action. The predicted resource objective covers a fixed window while the backlog/future response cohort is followed through modeled completion. Arrivals beyond the finite forecast horizon are omitted, so meeting the sampled constraint is not a service guarantee.

Two valid shadow cycles precede physical forecasting actions. A non-overlapping cadence skips missed slots rather than running concurrent controllers. State must be fresh from the start of collection and again before action; the completion timestamp still defines causal availability, and cutoff-to-action age has a separate bound. Measured decision age is added conservatively to predicted responses. Changed worker identities, readiness, admission state or assignment identities invalidate a proposal. A newly formed queue vetoes down selected from an empty-queue state, and new assignments on the selected worker require a fresh decision. Invalid forecasts permit only the declared fresh-state reactive up fallback, never an inferred down. The reactive comparison uses the same queue triggers, bounds and cooldown, with down requiring an empty queue and an empty worker on consecutive ticks.

The action journal persists intent before the Kubernetes patch. The patch tests worker identity and resource version. A missing acknowledgement is an uncertain outcome, not permission to retry: restart reconciliation compares the journal with actual admission state before another mutation. A namespace owner and local journal lock prevent duplicate controllers. Observed success requires a later snapshot confirming the expected worker state. This provides a reviewable demo recovery path; it is not general high-availability orchestration.

### Lifecycle occupancy and interpretation

Classifier CPU profiles alone understate occupied Job slots. After the configured warm-up, the loop freezes startup and release estimates with the profile selected at the first ready causal cutoff. Eligible completions through that recorded selection cutoff contribute to calibration; an unready initial cycle can delay it. Startup uses first-observed assignment to classifier start where assignment is not left-censored; release uses successful Job completion after classifier finish as a practical resource-release proxy. These are observation-dependent estimates, not exact scheduler or kernel timestamps. Zero-CPU startup/release fragments retain the requested application slot in native execution.

A Job still observed running after its selected profile ends remains explicit exhausted work. The model estimates a remaining duration from longer completions eligible at the frozen calibration cutoff, or a declared short fallback when no longer sample exists. That estimate preserves occupancy for hold/up evaluation, but never becomes observed completion evidence. Down stays unavailable in the presence of exhausted work. Persistent cordons retain ongoing assignments while excluding new queued and future admission, including at later cutoffs.

### Physical comparison

Pilot choices are frozen before independent held-out workload seeds. Fixed, reactive and forecast arms share planned offsets and payload sizes within each seed, with unique Job identities and rotated execution order. Warm-up backlog is reported separately from evaluation arrivals. Response follows original creation through bounded follow-up; failures and unfinished work remain in the deadline denominator, while response quantiles describe completed Jobs only. Allocation uses a common evaluation arrival window and includes draining workers. Follow-up completion does not silently enlarge that resource window.

Observation gaps yield allocation bounds rather than cheap partial totals. Unknown Job membership can hide draining work on cordoned workers but cannot hide the cost of known accepting workers. Paired savings retain these bounds and require matching plans, admission mode, resources and relative evaluation windows. Independent runs receive equal weight; repeated cutoffs and sampled futures remain dependent. Forecast-versus-observed cohort plots diagnose the running loop, whose later actions can alter outcomes. Separate matched physical arms establish benefit; neither an unchanged replay nor a successful API action validates an unperformed scaling counterfactual.
...
- Standalone OpenDC execution is not itself a controller; the repeated loop adds guarded observation, policy and actuation around the approximate live-state model.
...
Choosing which waiting Job runs next is distinct from choosing its worker. Similar aggregate throughput can conceal large per-Job response errors when the simulator and physical scheduler admit Jobs in different orders. Arrival-ordered lifecycle plots therefore compare waiting and execution directly for the same Job identities, retaining original creation times and the evaluation boundary. Aligning admission order is a practical route to reducing this error, but does not remove startup, resource-release or execution variability. The workload-specific FIFO admission mode creates each Job immediately but suspends its Pod admission. It releases Jobs in original creation-time/UID order, retaining one outstanding unbound reservation until Kubernetes placement is observed. The scheduler still enforces worker eligibility, cordons and resource fit; original Job timestamps continue to include admission waiting. A definite rejected patch triggers recollection, while an uncertain outcome or binding timeout stops further release. Reducing retry backoff or changing only the active-queue comparator does not by itself establish end-to-end FIFO. Lifecycle timestamps reveal overtaking, but identifying individual retry/backoff intervals requires scheduler evidence.
...
Configuration selection across independent workload seeds gives each run equal weight after matching eligible cutoffs within that run. Extra overlapping cutoffs and nested scenario prefixes do not create independent repetitions. Keep the scenario-sampling seed separate from the workload-generation seed. Freeze the choice before held-out execution; a shared native batch's process cost cannot be divided among configurations and presented as separately measured cost.

Controlled physical admission changes are evaluated against the action actually performed. Before/after API inventories establish which assigned Pods must remain and drain, while later scheduling timestamps establish new admission. The API request interval, observation lag and timestamp resolution leave boundary assignments explicitly inconclusive. Uncordoning a reserve demonstrates useful admission only when later work actually binds there. Kubernetes cordon does not switch off the VM, so these observations cannot validate simulated shutdown energy.
