...
import start as configuration_parser
...
To satisfy: (T_proc * R) < (C_e * Q_e)
    ...
    def verify(self):
        ...
        Tproc_norm = T_proc * (self.C_e * self.Q_e)
        logging.info("Normalized T_proc: %.4f", Tproc_norm)
        diff = ((Tproc_norm - self.T_proc) / self.T_proc) * 100
