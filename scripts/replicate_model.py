...
To satisfy: (T_proc / (C_e * Q_e)) < P
            ...
            %.2f < %.2f
            %s"""
            % (self.T_proc, self.C_e, self.Q_e, self.p, result, condition, satisfy)
        )
...
--------------------------------------------------""" % (
    ...
    def benchmark_normalize(self):
        ...
        df_endpoint["preproc/data (ms)"] = pd.to_numeric(
            df_endpoint["preproc/data (ms)"], downcast="float"
        )
...
To satisfy: (T_pre / (C_e * Q_e)) < P
            ...
            %.2f < %.2f
            %s"""
            % (self.T_pre, self.C_e, self.Q_e, self.p, result, condition, satisfy)
        )
...
def heatmap(local, offload):
    ...
    plt.hlines(
        y=func(cutoff_endpoint),
        xmin=cutoff_endpoint,
        xmax=xlim,
        color="k",
        linestyle="-",
        linewidth=3,
